...
const SubProjectsComponent = ({subProjects,deleteHandler}) => {
  ...
  return (
      ...
      <div className="w-full h-14 bg-blue-800 rounded-lg flex items-center justify-between px-2 lg:px-8 mb-2">
        <h1 className="text-sm lg:text-md text-white">All files are processed! Your widget is ready to go!</h1>
          ...
          <tbody className="w-full ">
            {
              subProjects.map((val,key)=>{
                return (
                      <tr key={key} className="h-[45px] font-semibold text-gray-700 mt-2 border-t-2 border-gray-200">
                        <td className=" text-center ">{val.filename}</td>
                        <td className=" text-center ">{dateAndTime(val.updatedAt)}</td>
                        <td className=" text-center ">{`${val.status?'Done':'Processing'}`}</td>
                        <td className="text-center">
                          <button className="w-1/4 h-8 border duration-200 border-r-0 rounded-l-lg border-gray-300 hover:bg-blue-500 hover:text-white" onClick={()=>{navigate(`/projects/edit/${val._id}`)}}>edit</button>
                          <button className="w-1/4 h-8 border duration-200 border-gray-300 rounded-r-lg hover:bg-red-600 hover:text-white" onClick={()=>{deleteHandler(val._id)}}>delete</button>
                        </td>
                      </tr>
                )
              })
