  ...
  return (
    <div className="absolute w-full h-full bg-black z-10 bg-opacity-40 flex justify-center items-center">
      <div className="w-3/5 h-2/3 lg:h-1/2  p-6 rounded-lg shadow-xl bg-white px-8 ">
        ...
        <input
          className="mt-1 w-full rounded text-black h-[50px] pl-4 bg-white border border-1 border-gray-400 focus:outline-none"
          value={uploadName}
          type="text"
        ...
        />
        <h1 className={`mt-1 text-sm text-red-600 ${error ? "" : "hidden"}`}>
          Need to fill both field before saving
        </h1>
        <div className="mt-6 w-full flex justify-end">
          <button
            className="ml-6 w-2/12 duration-200 font-semibold text-white h-10 rounded bg-[rgb(33,25,53)] hover:bg-[rgb(54,41,86)] "
            onClick={handleCreate}
          >
            {spinner ? <SpinnerComponents /> : "save"}
          </button>
        </div>
      </div>
    </div>
