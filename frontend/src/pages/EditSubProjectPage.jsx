      ...
      <div className="w-full md:w-9/12 h-[100vh] text-black px-10 pt-10 ">
        ...
        <div className="w-full mb-6 md:mb-0 flex flex-col md:flex-row justify-between items-center">
          ...
          <div className={`${edit ? "visible" : "hidden"} flex`}>
            <button
              className="w-28 h-10 rounded border border-red-400 text-red-400 hover:bg-red-600 hover:text-white font-semibold mr-4"
              onClick={handleDiscard}
            >
              Discard
            </button>
            <button
              className="w-28 h-10 rounded bg-[rgb(33,25,53)] font-semibold text-white hover:bg-[rgb(66,50,105)] mr-4"
              onClick={handleUpdate}
            >
              {updateLoading ? <SpinnerComponents /> : "Save & Exit"}
            </button>
          </div>
        </div>
        <div className="relative w-full h-3/5 md:h-2/3">
          ...
          />
          <button
            className={`absolute mt-6 ml-6 w-36 h-8 ${
              edit ? "bg-green-700" : "bg-[rgb(33,25,53)]"
            } hover:bg-[rgb(64,49,104)] rounded-full flex items-center justify-center text-white ${
              edit ? "visible" : "visible"
            }`}
            onClick={handleEdit}
          >
            <FaPen className="mr-3" />
            <span>{edit ? "Edit Mode" : "Enable"}</span>
          </button>
        </div>
      </div>
    </div>
