...
import { addWidgetConfiguration, getConfigurationData, getSubProjects } from "../services/API"
...
const WidgetConfigPage = () => {
    ...
    const handleImageUpload=(e)=>{
         ...
         if(!validFileTypes.find(type=> type===file.type)){
            console.log('file must be in jpg/jpeg/png format')
         }else{
    ...
    const form = new FormData()
      ...
      setisLoading(true)
      addWidgetConfiguration(form).then(res=>{
        setLink(res.data.link)
        setisLoading(false)
  ...
  return (
    <div className="w-full h-[100vh] flex flex-row text-black">
        ...
        <div className="w-full lg:w-9/12 h-[100vh] text-black px-10 pt-10 ">
          <ProjectHeaderComponent currentProject={currentProject.projectName} heading={'Configuration'} route={'/ widget configuration'}/>
          ...
          <div className={`w-1/5 flex items-center ${JSON.stringify(configData)===JSON.stringify(oldConfig)?'hidden':''}`}>
            <button className="-mt-10 w-3/4 h-10 bg-blue-800 hover:bg-blue-700 rounded-lg text-white" onClick={handleSubmit}>{isLoading?<SpinnerComponents/> :"Save Changes"}</button>
          </div>
          <hr className="absolute mt-9 border-1 border-gray-300 w-full rounded-2xl " />
          </div>
          {option===1?(<GeneralComponent setConfigData={setConfigData} configData={configData} error={error} />):''}
          {option===2?(<DisplayComponent setConfigData={setConfigData} configData={configData} error={error} link={link} handleImageUpload={handleImageUpload} />):''}
